...
import sys
import os
os.chdir(r"C:\Users\lorig\Desktop\PythonCode\Projects, Exercises, Programs\Trade Alert Journey")
...
# --------------------------------------------------------------------------------#
			...
			skewRegex = re.compile(r"('null'|-*\d*\.\d*)")
	...
	def update(self, chart, date = closestTradingDate(), amount = 1):
		...
		if chart == "oi":
			if isinstance(chart, str) == True:
				chart = [chart]
			url_list = self.URL_generator(charts = chart, start_date = date, quantity = 84)
